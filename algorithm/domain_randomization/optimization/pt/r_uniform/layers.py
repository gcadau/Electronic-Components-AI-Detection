...
class RandomBrightness(DRLayer):
    ...
    def forward(self, x, training=None, lower=-0.2, upper=0.2, value=None, rand=True):
        if not self._training(training):
            return x
        n = self._n(x)
        delta = self._uniform(n, lower, upper) if rand else self._values(value, n)
...
class RandomContrast(DRLayer):
    ...
    def forward(self, x, training=None, lower=0, upper=2.5, value=None, rand=True):
        if not self._training(training):
            return x
        n = self._n(x)
        contrast_factor = self._uniform(n, lower, upper) if rand else self._values(value, n)
...
class RandomHorizontallyFlip(DRLayer):
    ...
    def forward(self, x, training=None, lower=0, upper=1, value=None, rand=True):
        if not self._training(training):
            return x
        n = self._n(x)
        prob = self._uniform(n, lower, upper) if rand else self._values(value, n)
...
class RandomJpegQuality(DRLayer):
    ...
    def forward(self, x, training=None, lower=20, upper=100, value=None, rand=True):
        if not self._training(training):
            return x
        n = self._n(x)
        jpeg_quality = self._uniform(n, lower, upper) if rand else self._values(value, n)
...
class RandomSaturation(DRLayer):
    ...
    def forward(self, x, training=None, lower=0, upper=2, value=None, rand=True):
        if not self._training(training):
            return x
        n = self._n(x)
        saturation_factor = self._uniform(n, lower, upper) if rand else self._values(value, n)
